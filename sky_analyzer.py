...
def generate_sky_map_and_credible_region(ra_gw_samples, dec_gw_samples, nside=DEFAULT_NSIDE_SKYMAP, cdf_threshold=DEFAULT_PROB_THRESHOLD_CDF):
    ...
    if ra_gw_samples is None or dec_gw_samples is None or len(ra_gw_samples) == 0 or len(dec_gw_samples) == 0:
        print("⚠️ RA or Dec GW samples are empty or None. Cannot generate sky map.")
    ...
    if prob_map_gw.sum() > 0:
        ...
        print("⚠️ Warning: Sum of raw probability map is zero. No GW samples mapped effectively.")
        return prob_map_gw, empty_sky_mask, 0.0 # prob_map_gw will be all zeros

    print(f"Determining sky mask for {cdf_threshold*100:.0f}% credible region...")
    ...
    try:
        ...
        if cdf_threshold == 1.0 and len(sorted_probs) > 0:
            ...
            print(f"⚠️ Could not determine sky map probability threshold for CDF {cdf_threshold}. Using 0.0.")
    ...
    print(f"  Probability threshold for this CR: {sky_map_threshold_val:.2e}")
...
def select_galaxies_in_sky_region(all_galaxies_df, sky_mask_boolean, nside=DEFAULT_NSIDE_SKYMAP):
    ...
    if all_galaxies_df.empty:
        print("⚠️ Input galaxy catalog is empty. Cannot perform spatial selection.")
        return empty_df

    if not all(col in all_galaxies_df.columns for col in ['ra', 'dec']):
        print("⚠️ 'ra' or 'dec' column missing in all_galaxies_df. Cannot perform spatial selection.")
        return empty_df
        
    if sky_mask_boolean is None or sky_mask_boolean.size != hp.nside2npix(nside):
        print("⚠️ Sky mask is invalid or Nside mismatch. Cannot perform spatial selection.")
        return empty_df

    print(f"Spatially selecting galaxies within the provided sky mask (Nside={nside})...")
    ...
    try:
        ...
        print(f"  Selected {len(spatially_selected_galaxies_df):,} galaxies falling within the sky mask.")
    except IndexError as e:
        print(f"❌ IndexError during spatial selection of galaxies: {e}. Likely an issue with gpix_gal indices.")
        print(f"  Max gpix_gal: {gpix_gal.max() if len(gpix_gal) > 0 else 'N/A'}, sky_mask_boolean size: {sky_mask_boolean.size}")
        return empty_df
...
def filter_galaxies_by_redshift(galaxies_df, z_max_filter):
    ...
    if galaxies_df.empty:
        print("Input DataFrame for redshift filtering is empty.")
        return pd.DataFrame()

    if 'z' not in galaxies_df.columns:
        print("⚠️ Column 'z' missing in galaxies_df. Cannot apply redshift cut.")
        return galaxies_df # Or an empty DF, depending on desired strictness. Returning original for now.

    print(f"Applying redshift cut z < {z_max_filter}...")
    original_count = len(galaxies_df)
    candidate_hosts_df = galaxies_df[galaxies_df['z'] < z_max_filter].copy()
    print(f"  → {len(candidate_hosts_df):,} galaxies remaining after z < {z_max_filter} cut (from {original_count}).")
    
    return candidate_hosts_df


if __name__ == '__main__':
    print("--- Testing sky_analyzer.py ---")
    ...
    if prob_map.sum() > 0 and mask.any():
        print(f"  Successfully generated map (sum: {prob_map.sum():.2f}) and mask ({mask.sum()} pixels in 50% CR). Threshold: {thresh_val:.2e}")
        assert prob_map.size == hp.nside2npix(mock_nside)
        assert mask.size == hp.nside2npix(mock_nside)
    else:
        print("  Test 1 FAILED or map was empty.")
    ...
    if mask.any(): # Proceed only if mask is valid
        ...
        if not spatially_selected.empty:
            print(f"  Spatially selected {len(spatially_selected)} galaxies:")
            print(spatially_selected)
            ...
            print("  No galaxies were spatially selected. This might be expected or indicate an issue with map/catalog alignment.")
        else:
            print("  Test 2: Spatially selected DataFrame is empty (and no GW samples to begin with).")
    else:
        print("  Test 2 SKIPPED as the mask from Test 1 was empty or invalid.")
    ...
    if not z_filtered_galaxies.empty:
        print(f"  Filtered {len(z_filtered_galaxies)} galaxies with z < 0.1:")
        print(z_filtered_galaxies)
        assert all(z < 0.1 for z in z_filtered_galaxies['z'])
        assert 5 not in z_filtered_galaxies['PGC'].values # PGC 5 has z=0.15
    else:
        print("  Test 3 FAILED or resulted in an empty DataFrame post-filtering.")
    ...
    if 'spatially_selected' in locals() and not spatially_selected.empty:
        ...
        if not final_candidates.empty:
            print(f"  Final candidates (spatially selected & z < 0.01): {len(final_candidates)} galaxies")
            print(final_candidates)
            assert all(z < 0.01 for z in final_candidates['z'])
        else:
            print("  No final candidates after redshift cut on spatially selected galaxies.")
    else:
        print("  Test 3b SKIPPED as no spatially selected galaxies were available from Test 2.")
    ...
    print("  Test 4a (None GW samples) PASSED.")
    ...
    print("  Test 4b (Empty list GW samples) PASSED.")
    ...
    print("  Test 5a (Empty galaxy DF) PASSED.")
    ...
    print("  Test 5b (Invalid mask) PASSED.")
    ...
    print("  Test 5c (Missing ra/dec cols) PASSED.")
    ...
    print("  Test 6a (Empty DF for z filter) PASSED.")
    ...
    print("  Test 6b (Missing z col) PASSED (returned original).")
