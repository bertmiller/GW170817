...
def extract_gw_event_parameters(data_object, event_name):
    ...
    """
    logger.info(f"Extracting dL, RA, Dec for {event_name} from provided data object...")
    try:
        ...
        if hasattr(data_object, 'samples_dict') and data_object.samples_dict:
            ...
            logger.debug(f"  Extracted samples_dict from PESummary object attributes for {event_name}.")
            ...
            logger.debug(f"  Using provided data object directly as samples_dict for {event_name}.")
        else:
            logger.error(f"❌ Data for {event_name} is not a recognized PESummary object or samples dictionary. Type: {type(data_object)}")
        ...
        if all(k in samples_dict_top_level for k in ["luminosity_distance", "ra", "dec"]):
            ...
            logger.debug(f"  Found target keys directly in top-level samples_dict for {event_name}.")
            ...
            if preferred_keys:
                ...
                logger.info(f"  Multiple analyses found in samples_dict for {event_name}. Using: '{chosen_key}'.")
                ...
                logger.info(f"  Multiple analyses found in samples_dict for {event_name}. Using first one (fallback): '{first_key}'.")
            else:
                logger.warning(f"  Could not identify a suitable sub-dictionary with all target keys in samples_dict for {event_name}.")
                logger.debug(f"  Available top-level keys: {list(samples_dict_top_level.keys())}") # Log available keys for diagnosis
                return None, None, None
        else:
            logger.error(f"❌ Samples dictionary for {event_name} is empty or None.")
            return None, None, None
        ...
        if actual_samples and all(k in actual_samples for k in ["luminosity_distance", "ra", "dec"]):
            ...
            logger.info(f"  Successfully extracted and processed {len(dL_samples)} dL, RA (deg), Dec (deg) samples for {event_name}.")
            ...
            if actual_samples:
                logger.debug(f"  Available keys in actual_samples: {list(actual_samples.keys())}")
            else:
                logger.debug("  actual_samples was None.")
            return None, None, None
            
    except KeyError as e:
        logger.error(f"❌ Key Error during extraction for {event_name}: Key {e} not found.")
        # Consider logging the structure of actual_samples if it exists
        try:
            if 'actual_samples' in locals() and actual_samples is not None:
                 logger.debug(f"    Available keys in actual_samples where error occurred: {list(actual_samples.keys())}")
            elif 'samples_dict_top_level' in locals() and samples_dict_top_level is not None:
                 logger.debug(f"    Available keys in samples_dict_top_level where error occurred: {list(samples_dict_top_level.keys())}")
        except Exception as log_e:
            logger.debug(f"    (Additionally, error logging keys: {log_e})")
        return None, None, None
    except Exception as e:
        logger.exception(f"❌ An unexpected error occurred extracting samples for {event_name}: {e}")
        return None, None, None

if __name__ == '__main__':
    import sys # For StreamHandler in basicConfig
    logging.basicConfig(
        level=logging.DEBUG, 
        format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
        handlers=[logging.StreamHandler(sys.stdout)]
    )
    logger.info("--- Testing event_data_extractor.py ---")
    ...
    if dL is not None and ra is not None and dec is not None:
        logger.info(f"  Extracted Case 1: dL_mean={np.mean(dL):.1f}, RA_mean={np.mean(ra):.1f} deg, Dec_mean={np.mean(dec):.1f} deg")
        assert len(dL) == 3
        assert abs(np.mean(ra) - (np.degrees(np.pi/4) + np.degrees(np.pi/3) + np.degrees(np.pi/2))/3) < 1e-6
    else:
        logger.error("  Case 1 FAILED")
    ...
    if dL is not None and ra is not None and dec is not None:
        logger.info(f"  Extracted Case 2: dL_mean={np.mean(dL):.1f}, RA_mean={np.mean(ra):.1f} deg, Dec_mean={np.mean(dec):.1f} deg")
        assert len(dL) == 2
    ...
    if dL is not None and ra is not None and dec is not None:
        logger.info(f"  Extracted Case 3: dL_mean={np.mean(dL):.1f}, RA_mean={np.mean(ra):.1f} deg, Dec_mean={np.mean(dec):.1f} deg")
        assert len(dL) == 4
    else:
        logger.error("  Case 3 FAILED")
    ...
    if dL is None and ra is None and dec is None:
        logger.info("  Case 4 correctly returned None for missing 'ra'.")
    else:
        logger.error("  Case 4 FAILED - should have returned None.")
    ...
    if dL is None and ra is None and dec is None:
        logger.info("  Case 5 correctly returned None for empty data.")
    else:
        logger.error("  Case 5 FAILED - should have returned None.")
    ...
    if dL is None and ra is None and dec is None:
        logger.info("  Case 6 correctly returned None for invalid data type.")
    else:
        logger.error("  Case 6 FAILED - should have returned None.")
    ...
    if dL is None and ra is None and dec is None:
        logger.info(f"  Case 7 correctly returned None due to empty samples_dict.")
    else:
        logger.error("  Case 7 FAILED")
    ...
    if dL is None and ra is None and dec is None:
        logger.info(f"  Case 8 correctly returned None due to sub-dict missing essential keys.")
    else:
        logger.error("  Case 8 FAILED")
